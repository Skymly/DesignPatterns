...
public sealed class DiRegistrationMapTests
{
    ...
    [Fact]
    public void Build_skips_registrations_in_generated_file_path_trees()
    {
        ...
        const string generatedSource = """
            ...
            class GeneratedSingletonService { }

            static class GeneratedStartup
            {
                public static void Configure(IServiceCollection services)
                {
                    services.AddSingleton<GeneratedSingletonService>();
                }
            }
            """;

        var map = DiRegistrationMap.Build(CreateCompilation(
            ("Startup.cs", userSource),
            ("Registry.g.cs", generatedSource)));

        Assert.Equal(Lifetime.Scoped, GetLifetime(map, "UserScopedService"));
        Assert.DoesNotContain(map.Lifetimes.Keys, t => t.Name == "GeneratedSingletonService");
    }

    [Fact]
    public void Build_skips_registrations_in_trees_with_auto_generated_header()
    {
        const string userSource = """
            using Microsoft.Extensions.DependencyInjection;

            class UserTransientService { }

            static class Startup
            {
                public static void Configure(IServiceCollection services)
                {
                    services.AddTransient<UserTransientService>();
                }
            }
            """;
        ...
        const string generatedSource = """
            ...
            class HeaderMarkedSingleton { }

            static class HeaderMarkedStartup
            {
                public static void Configure(IServiceCollection services)
                {
                    services.AddSingleton<HeaderMarkedSingleton>();
                }
            }
            """;
        ...
        var map = DiRegistrationMap.Build(CreateCompilation(
            ("Startup.cs", userSource),
            ("Something.cs", generatedSource)));

        Assert.Equal(Lifetime.Transient, GetLifetime(map, "UserTransientService"));
        Assert.DoesNotContain(map.Lifetimes.Keys, t => t.Name == "HeaderMarkedSingleton");
    }
    ...
    private static CSharpCompilation CreateCompilation(string source) =>
        CreateCompilation(("Test.cs", source));
