...
| 生成代码质量提升 | `#nullable enable` + `[GeneratedCode]` + `WithTrackingName` 已完成；剩余：所有生成器输出加 `/// <summary>` XML 文档 | — | [~] |
...
| EventAggregator 发布追踪 | `PublishTracedAsync` + `EventPublicationTrace`；~~handler 错误聚合模式（`StopOnError` / `ContinueOnError` / `AggregateErrors`）~~（已完成，PR #166） | — | [~] |
...
| 生成代码缺 nullable/文档/标记 | ~~生成器输出无 `#nullable enable`、无 `[GeneratedCode]`、无 `/// <summary>` XML 文档~~ `#nullable enable` + `[GeneratedCode]` + `WithTrackingName` 已完成（commit b9a650e）；剩余：生成成员 `/// <summary>` XML 文档 | 所有生成器统一加 nullable 头 + `[GeneratedCode]` + XML 文档；增量管线加 `WithTrackingName` 优化缓存 | [~] |
