...
| `CompositeTraverser` | 同步/异步遍历单根树或森林 |
...
| `CompositeCatalogAssembler` | 从 catalog 装配单根（`Assemble`）或多根森林（`AssembleForest`） |
...
当 catalog 含多个 `ParentKey == null` 的根时，使用 `BuildForest()` 与 `TraverseForest`：

```csharp
[CompositePart<IMenuNode>("home")]
public sealed class HomeMenu : IMenuNode, ICompositeBuildable<IMenuNode> { ... }

[CompositePart<IMenuNode>("admin", Order = 10)]
public sealed class AdminMenu : IMenuNode, ICompositeBuildable<IMenuNode> { ... }

[CompositePart<IMenuNode>("settings", ParentKey = "home", Order = 0)]
public sealed class SettingsMenu : IMenuNode, ICompositeBuildable<IMenuNode> { ... }

var forest = MenuNodeCompositeCatalog.BuildForest();

CompositeTraverser.TraverseForest(forest, (node, depth, rootIndex) =>
    Console.WriteLine($"root={rootIndex} depth={depth} {node.Title}"));
```
...
var entries = new[]
{
    new CompositeCatalogEntry<IMenuNode>("home", null, 0, typeof(HomeMenu)),
    new CompositeCatalogEntry<IMenuNode>("admin", null, 10, typeof(AdminMenu)),
    new CompositeCatalogEntry<IMenuNode>("settings", "home", 0, typeof(SettingsMenu)),
};

var forest = CompositeCatalogAssembler.AssembleForest(entries);
```

`BuildRoot()` 仍要求 catalog 中**恰好一个**根；多根 catalog 调用 `BuildRoot()` 会在运行时抛 `CompositeAssemblyException`。
...
| 单根（`BuildRoot`） | catalog 中恰好一个 `ParentKey == null` |
| 多根（`BuildForest`） | 一个或多个 `ParentKey == null`，按 `Order` 再 key 排序 |
...
- `{Contract}CompositeCatalog` — entry 列表 + `BuildRoot()` + `BuildForest()`
...
- 运行时动态增删节点
- DI 自动注册 `BuildRoot()` / `BuildForest()` 结果
