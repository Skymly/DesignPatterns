...
`App.config` / `ConfigurationManager` 桥接由 `DesignPatterns.Extensions.Configuration` 提供（`RegistryConfiguration.ResolveConfigured` / `TryResolveConfigured`，见 [Configuration.md](Configuration.md)）。
