...
| `DesignPatterns.Extensions.Configuration/` | AppSettings → strategy registry 桥接 |
...
| Analyzer | `tests/DesignPatterns.Analyzers.Tests` | DP006、DP023、DP024、DP025 与 CodeFix（Strategy/Factory/Handler/Registry key） |
| DI 扩展 | `tests/DesignPatterns.Extensions.DependencyInjection.Tests` | DI 注册与解析 |
| Configuration 扩展 | `tests/DesignPatterns.Extensions.Configuration.Tests`（net8.0）、`tests/DesignPatterns.Extensions.Configuration.Tests.Net48`（net48 编译） | AppSettings 桥接 |
