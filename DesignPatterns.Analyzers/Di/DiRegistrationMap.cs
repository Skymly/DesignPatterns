...
/// <summary>
/// DI registration map: type→lifetime view built from explicit container
/// registrations (MSDI Add*/TryAdd, Autofac RegisterType/Register) and
/// attributed <c>RegisterDi</c> expansions (holder-category matching), plus
/// Singleton factory-delegate registrations for DP066.
/// </summary>
/// <remarks>
/// Consumers obtain a map only via <see cref="Build"/>. Incremental collection
/// is a private implementation detail of that entry point.
/// </remarks>
internal sealed class DiRegistrationMap
{
    ...
    public static DiRegistrationMap Build(Compilation compilation)
    {
        ...
        var builder = new Builder(attributedTypes);
    ...
    private static DiRegistrationMap FromEntries(
    ...
    /// <summary>
    /// Private collector used only by <see cref="Build"/>.
    /// </summary>
