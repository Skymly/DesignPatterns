...
/// <see cref="Build"/> skips generated syntax trees so callers that configure
/// <c>GeneratedCodeAnalysisFlags.None</c> keep the same registration visibility
/// they had with <c>RegisterSyntaxNodeAction</c>.
/// </remarks>
internal sealed class DiRegistrationMap
{
    ...
    /// <summary>
    /// Walks invocation expressions in non-generated syntax trees of
    /// <paramref name="compilation"/> and collects explicit container
    /// registrations plus attributed <c>RegisterDi</c> expansions into a map.
    /// </summary>
    ...
    /// <summary>
    /// Mirrors the generated-code heuristics Roslyn applies when analyzers use
    /// <c>GeneratedCodeAnalysisFlags.None</c> with syntax-node actions: generated
    /// file-name suffixes and leading <c>auto-generated</c> comments.
    /// </summary>
    private static bool IsGeneratedSyntaxTree(SyntaxTree tree)
    {
        ...
        if (root.HasLeadingTrivia && BeginsWithAutoGeneratedComment(root.GetLeadingTrivia()))
        {
            return true;
        }

        // File-scoped / first-token trivia when the root itself has none.
        var firstToken = root.GetFirstToken(includeZeroWidth: true);
        return firstToken != default
            && BeginsWithAutoGeneratedComment(firstToken.LeadingTrivia);
    }
    ...
    private static bool BeginsWithAutoGeneratedComment(SyntaxTriviaList triviaList)
    {
        foreach (var trivia in triviaList)
        {
            ...
            // First non-whitespace trivia is not an auto-generated comment.
            return false;
        }
