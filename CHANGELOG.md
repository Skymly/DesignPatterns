...
- **Command Router stream generator**: `[RegisterCommandHandler]` on `IStreamCommandHandler<TCommand, TItem>` drives `{Command}CommandHandlerRegistry` binding for `SendStreamAsync` (static + DI `RegisterDi` parity); stream + non-stream on the same command remains **DP073**; **DP074** message now includes the stream contract; no stream pipeline behaviors ([#265](https://github.com/Skymly/DesignPatterns/issues/265)).
- **Command Router stream send (runtime)**: `IStreamCommandHandler<TCommand, TItem>`, `ICommandRouter.SendStreamAsync` / `TrySendStreamAsync`, and `CommandRouterBuilder.Register` for stream handlers; netstandard2.0 via `Microsoft.Bcl.AsyncInterfaces`; shared 1:1 bijection with void/result handlers; no stream pipeline behaviors ([#282](https://github.com/Skymly/DesignPatterns/issues/282)).
- **Command Router pipeline behavior generator**: `[CommandPipelineBehavior]` / `[CommandPipelineBehavior<TCommand>]` (AllowMultiple; lower order outermost) drives `RegisterCommandHandlerGenerator` to emit ordered `UseBehavior` calls alongside the terminal handler in `{Command}CommandHandlerRegistry.RegisterAll` / `CreateRouter` (static parameterless-ctor path only; no behavior `RegisterDi`). Reports **DP075** (duplicate order), **DP076** (orphan behavior), and **DP077** (contract mismatch) ([#264](https://github.com/Skymly/DesignPatterns/issues/264)).
- **Command Router pipeline behavior diagnostic IDs**: reserved **DP075** (duplicate behavior order), **DP076** (orphan behavior with no terminal handler), and **DP077** (behavior contract mismatch) for the upcoming `[CommandPipelineBehavior]` generator; DP067–DP071 remain ADR-008-only ([#276](https://github.com/Skymly/DesignPatterns/issues/276)).
- **Command Router pipeline behaviors (runtime)**: Chain-like `next` onion via `ICommandPipelineBehavior<TCommand>` / `ICommandPipelineBehavior<TCommand, TResult>` and `CommandPipelineDelegate`; `CommandRouterBuilder.UseBehavior` registers ordered per-command behaviors (lower order outermost) frozen at `Build`; MediatR-shaped short-circuit (void omits `next`; result returns own value or `await next`). Manual/static only — no DI, attributes, or diagnostics in this slice ([#275](https://github.com/Skymly/DesignPatterns/issues/275)).
- **ADR-009**: Command Router pipeline behaviors use a Chain-like `next` onion (not Decorator wrap); dual void/`TResult` behavior contracts; per-command closed registration; MediatR-shaped short-circuit; dedicated types (no `HandlerPipeline` reuse); ticket split #275–#277 / #264 ([#277](https://github.com/Skymly/DesignPatterns/issues/277)).
- **Command Router Autofac**: `RegisterCommandRouter` on `ContainerBuilder` mirrors MSDI `AddCommandRouter` (configure delegate + `ILifetimeScope`, default `InstanceSharing.Shared` / `IfNotRegistered`); pairs with generated `{Command}CommandHandlerRegistry.RegisterAutofac` / `RegisterAll(..., ILifetimeScope)`; Autofac integration tests prove void and result `SendAsync` ([#263](https://github.com/Skymly/DesignPatterns/issues/263)).
- **Command Router Design Doc**: documented MVP runtime / attribute / generator / diagnostics (DP072–074) / DI surface, comparison vs Event Aggregator (1:N) and MediatR, and pipeline/stream as future in-domain capabilities; indexed under `docs/design/` ([#260](https://github.com/Skymly/DesignPatterns/issues/260)).
- **Command Router DI**: `AddCommandRouter` registers `ICommandRouter` via a configure delegate that receives `CommandRouterBuilder` + `IServiceProvider`; pairs with generated `{Command}CommandHandlerRegistry.RegisterDi` (default Transient handlers) and documents Singleton-router captive capture relative to DP060–DP062 / DP066 ([#262](https://github.com/Skymly/DesignPatterns/issues/262)).
- **Command Router unregistered-handler Analyzer + CodeFix**: **DP072** reports concrete `ICommandHandler<T>` / `ICommandHandler<T,TResult>` implementations that lack `[RegisterCommandHandler]` when a peer registration for the same command type exists; CodeFix inserts the generic attribute form on C# 11+ when available, otherwise `typeof` ([#261](https://github.com/Skymly/DesignPatterns/issues/261)).
