...
> - **NuGet 发布**：元包 **`Skymly.DesignPatterns`** **`0.1.0-preview6`** 发布至 [nuget.org](https://www.nuget.org/packages/Skymly.DesignPatterns) 与 GitHub Packages；亦支持本地 `./build.ps1 --target CiPack`。C# 命名空间仍为 `DesignPatterns.*`；旧 GitHub 包 ID `DesignPatterns`（preview1/2）已弃用。
...
### NuGet（预览，`Skymly.DesignPatterns` `0.1.0-preview6`）



```xml
<PackageReference Include="Skymly.DesignPatterns" Version="0.1.0-preview6" />
```
