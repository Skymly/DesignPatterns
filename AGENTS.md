...
| **NuGet** | 元包 **`Skymly.DesignPatterns`**；当前 **`0.2.1-preview1`**；`release.yml` + Nuke `Publish` → nuget.org + GitHub Packages |
...
- 下一个可用 ID：**DP062**；ID 一经发布不复用、不改语义。
...
- 归属：DP006 / DP023 / DP024 / DP025 / DP033 / DP036 / DP044 / DP060 / DP061 属 **Analyzer**；其余属**生成器**。
...
- 版本号：`VersionPrefix` + `VersionSuffix`（当前 `0.2.1-preview1`），单一真相源为 [`Directory.Build.props`](Directory.Build.props)；发版时可通过环境变量 `VERSION` 覆盖。
