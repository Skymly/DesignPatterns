...
| **NuGet** | 元包 **`Skymly.DesignPatterns`**；当前 **`0.2.0-preview3`**；`release.yml` + Nuke `Publish` → nuget.org + GitHub Packages |
...
- 版本号：`VersionPrefix` + `VersionSuffix`（当前 `0.2.0-preview3`），单一真相源为 [`Directory.Build.props`](Directory.Build.props)；发版时可通过环境变量 `VERSION` 覆盖。
